...
class TestFileAttachment:
    ...
    def test_file_attachment_creation(self):
        ...
        attachment = FileAttachment(name="file1", filename="test.txt", content_type="text/plain", data=b"test content")
    ...
    def test_file_attachment_optional_filename(self):
        ...
        attachment = FileAttachment(
            name="field1", filename=None, content_type="application/octet-stream", data=b"binary data"
        )
...
class TestBackendConnector:
    ...
    def test_init_custom_parameters(self, mock_https_connection):
        ...
        mock_https_connection.assert_called_once_with(host="custom.example.com", port=8080, timeout=30.0)
    ...
    def test_init_with_gzip_enabled(self, mock_https_connection):
        ...
        connector = BackendConnector(host="api.example.com", default_headers=custom_headers, accept_gzip=True)

        expected_headers = {"User-Agent": "test", "Accept-Encoding": "gzip"}
    ...
    def test_request_success(self, mock_log, mock_time, mock_https_connection):
        ...
        # Verify calls
        mock_conn.request.assert_called_once_with("GET", "/test", body=None, headers={"Accept-Encoding": "gzip"})
        assert response == mock_response
        assert response_data == b"response data"
        mock_log.debug.assert_called_once_with("Request to %s %s took %.3f seconds", "GET", "/test", 1.5)
    ...
    def test_request_with_gzip_compression(self, mock_gzip_compress, mock_time, mock_https_connection):
        ...
        mock_conn.request.assert_called_once_with(
            "POST", "/test", body=b"compressed data", headers={"Accept-Encoding": "gzip", "Content-Encoding": "gzip"}
        )
    ...
    def test_request_with_custom_headers(self, mock_https_connection):
        ...
        expected_headers = {"Accept-Encoding": "gzip", "Authorization": "Bearer token", "Custom-Header": "value"}
        mock_conn.request.assert_called_once_with("GET", "/test", body=None, headers=expected_headers)
    ...
    def test_post_files_single_file(self, mock_uuid, mock_https_connection):
        ...
        # Test post_files
        connector = BackendConnector(host="api.example.com")
        files = [FileAttachment(name="file1", filename="test.txt", content_type="text/plain", data=b"file content")]
    ...
    def test_post_files_no_filename(self, mock_uuid, mock_https_connection):
        ...
        connector = BackendConnector(host="api.example.com")
        files = [FileAttachment(name="data", filename=None, content_type="application/json", data=b'{"key": "value"}')]
